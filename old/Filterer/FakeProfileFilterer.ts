...
export class FakeProfileFilterer implements IProfileFilterer {
  name: string = NAME

  check(profile: IProfile): boolean {
    const followersCountScore = FakeProfileFilterer.followersCountScore(profile)
    const followingCountScore = FakeProfileFilterer.followingCountScore(profile)
    const followersFollowingRatioScore = FakeProfileFilterer.followersFollowingRatioScore(profile)
    const postsCountScore = FakeProfileFilterer.postsCountScore(profile)
    const hasClipsScore = FakeProfileFilterer.hasClipsScore(profile)
    const hasChannelScore = FakeProfileFilterer.hasChannelScore(profile)
    const highlightReelCountScore = FakeProfileFilterer.highlightReelCountScore(profile)
    const igtvCountScore = FakeProfileFilterer.igtvCountScore(profile)
    const postsAnalysisScore = FakeProfileFilterer.postsAnalysisScore(profile)
